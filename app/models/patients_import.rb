class PatientsImport
  ...
  def load_imported_items
    ...
    (2..spreadsheet.last_row).map do |i|
      ...
      item = Patient.find_by_id(row["id"]) || Patient.new
      if row.to_hash.keys.difference(Patient.columns.collect(&:name)).any?
        item.errors.add(:base, "Content is invalid!")
      else
        item.attributes = row.to_hash.merge(user_id: user_id)
      end
